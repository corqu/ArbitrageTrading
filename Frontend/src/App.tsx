...
const App: React.FC = () => {
  ...
  const [selectedDomesticExchange, setSelectedDomesticExchange] = useState<'UPBIT' | 'BITHUMB'>('UPBIT');
  const [selectedForeignExchange, setSelectedForeignExchange] = useState<'BINANCE' | 'BYBIT'>('BINANCE');
  ...
  useEffect(() => {
    ...
    return () => { stompClient.deactivate(); if (logoutTimerRef.current) window.clearTimeout(logoutTimerRef.current); };
  }, [selectedDomesticExchange, selectedForeignExchange]);

  const handleUpdateProfile = async (e: React.FormEvent) => {
    e.preventDefault();
    try {
      await axios.put('/api/auth/profile', { nickname: newNickname, password: newPassword });
      alert('프로필이 수정되었습니다.');
  ...
  const handleConnectExchange = async (e: React.FormEvent) => {
    e.preventDefault();
    try {
      await axios.post('/api/user/credentials/register', { exchange: selectedExchange, accessKey: apiKey, secretKey: apiSecret });
  ...
  const handleSignup = async (e: React.FormEvent) => {
    ...
    try {
      ...
      alert('회원가입 완료');
      setAuthMode('login');
    } catch (err: any) { setLoginError(err.response?.data || '회원가입 실패'); }
    finally { setIsAuthLoading(false); }
  };

  const checkNicknameAvailability = async () => {
    try {
      const res = await axios.get(`/api/auth/check-nickname?nickname=${nickname}`);
      setIsNicknameAvailable(res.data.available);
      setIsNicknameChecked(true);
    } catch (err) {}
  };

  if (loading) return <div className="dashboard-loading"><RefreshCw className="animate-spin" size={48} /><p>서버 연결 중...</p></div>;
          ...
          <Routes>
            ...
            <Route path="/auth" element={
              ...
              <AuthPage 
                ...
                isNicknameChecked={isNicknameChecked} isNicknameAvailable={isNicknameAvailable}
                setIsNicknameChecked={setIsNicknameChecked} setIsNicknameAvailable={setIsNicknameAvailable}
                loginError={loginError} isAuthLoading={isAuthLoading}
              />
            } />
            <Route path="/mypage" element={
              isLoggedIn ? <MyPage 
                ...
                connectedExchanges={connectedExchanges} handleDeleteExchange={handleDeleteExchange}
                showAddExchangeModal={showAddExchangeModal} setShowAddExchangeModal={setShowAddExchangeModal}
