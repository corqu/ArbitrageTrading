...
interface MyPageProps {
  ...
  setSelectedExchange: (
    val: "UPBIT" | "BITHUMB" | "BINANCE" | "BYBIT" | null,
  ) => void;
  apiKey: string;
...
const MyPage: React.FC<MyPageProps> = ({
  ...
  const fetchAssetData = async (
    ...
    const isConnected = connectedExchanges.some(
      (ex) => ex.toUpperCase() === exchange.toUpperCase(),
    );
    if (!isConnected) {
      setAssetData(null);
      return;
    ...
    try {
      const res = await axios.get(
        `/api/user/credentials/assets?exchange=${exchange}`,
      );
        ...
        >
          <div style={{ display: "flex", alignItems: "center", gap: "0.6rem" }}>
            ...
            <select
              ...
              onChange={(e) =>
                isDomestic
                  ? setDomesticExchange(e.target.value as any)
                  : setForeignExchange(e.target.value as any)
              }
              style={{
            ...
            >
              {isDomestic ? (
                <>
                  <option value="UPBIT" style={{ background: "#1a1a1a" }}>
                    UPBIT
                  </option>
                  <option value="BITHUMB" style={{ background: "#1a1a1a" }}>
                    BITHUMB
                  </option>
                </>
              ) : (
                <>
                  <option value="BINANCE" style={{ background: "#1a1a1a" }}>
                    BINANCE
                  </option>
                  <option value="BYBIT" style={{ background: "#1a1a1a" }}>
                    BYBIT
                  </option>
                </>
              )}
            ...
            >
              ...
              <div
                style={{
                  fontSize: "1.2rem",
                  fontWeight: 800,
                  color: !isDomestic ? "#f3ba2f" : "var(--accent-color)",
                }}
              >
                {data.mainBalance?.toLocaleString(undefined, {
    ...
    >
      <div className="card" style={{ padding: "2.5rem" }}>
        ...
        {isEditingProfile && (
          ...
          >
            <h4 style={{ marginTop: 0, marginBottom: "1.5rem" }}>
              회원 정보 수정
            </h4>
            <div
              style={{
        ...
        >
          ...
          {showAddExchangeModal && (
            ...
            >
              ...
              >
                <span style={{ fontWeight: 700 }}>새 거래소 연동</span>
                <button
                  onClick={() => {
                ...
                {["UPBIT", "BITHUMB", "BINANCE", "BYBIT"].map((ex) => (
                  <button
                    ...
                    style={{
                      ...
                      padding: "0.6rem",
                      borderRadius: "0.4rem",
                      border: `1px solid ${selectedExchange === ex ? "var(--accent-color)" : "var(--border-color)"}`,
                      background:
                        selectedExchange === ex
                          ? "rgba(56, 189, 248, 0.1)"
                          : "transparent",
        ...
        >
          ...
          >
            <RefreshCw
              size={14}
              className={
                isLoadingDomestic || isLoadingForeign ? "animate-spin" : ""
              }
            />
            새로고침
          </button>
