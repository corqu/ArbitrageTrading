...
import { 
  RefreshCw, BarChart3, ChevronDown, ChevronUp, Search
} from 'lucide-react';
...
const Dashboard: React.FC<DashboardProps> = ({ 
  ...
  const fetchHistory = async (symbol: string, range: string) => {
    ...
    try {
      ...
      const response = await axios.get(`/api/kimp/history?symbol=${symbol}&range=${range}&domesticExchange=AVERAGE&foreignExchange=${foreignEx}`);
      ...
      <header className="main-header" style={{ alignItems: 'center', flexWrap: 'wrap', gap: '1rem' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '2rem' }}>
          <div className="header-title">
            <h2>시장 대시보드</h2>
            <p className="subtitle">전체 {filteredList.length}개 코인 실시간 현황</p>
          </div>
                ...
                <React.Fragment key={`${item.symbol}-${idx}`}>
                  <tr 
                    onClick={() => { 
                      ...
                      else { setSelectedSymbol(item.symbol); fetchHistory(item.symbol, selectedRange); } 
                    }} 
                    ...
                    <tr className="expanded-row">
                      <td colSpan={5}>
                        <div className="expanded-content">
                          ...
                          <div className="chart-wrapper" style={{ height: '250px', width: '100%' }}>
                            {loadingHistory ? <div style={{ textAlign: 'center', padding: '2rem' }}><RefreshCw className="animate-spin" /></div> : (
